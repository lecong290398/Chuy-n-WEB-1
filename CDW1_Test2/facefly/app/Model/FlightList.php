...
class FlightList extends Model
{
    ...
    // Query ID search 
    public function getId_search($fl_route , $fl_ct_from, $fL_ct_to , $fl_class , $fl_type, $fl_total,$fl_depar_time,$fl_return_time)
    {
      switch ($fl_type) 
      {
        case 1:
            $values  = FlightList::where ([
              ['fl_flight_route_id', $fl_route],
              ['fl_city_from_id' , $fl_ct_from] ,
              ['fl_city_to_id',$fL_ct_to],
              ['fl_fc_id', $fl_class],
              ['fl_type',  $fl_type],
              ['fl_total',$fl_total],
              ['fl_departure_date','>=', $fl_depar_time],
              ['fl_departure_date', '<=',$fl_depar_time + 86400],
              ['fl_return_date',$fl_return_time]])
        ...
        case 0:
            $values = FlightList::where ([
                    ...
                    ['fl_departure_date', '<=',$fl_depar_time + 86400],
                    ['fl_return_date','>=',$fl_return_time],
                    ['fl_return_date', '<=',$fl_return_time + 86400]
                  ])
                  ->join('flight_classes','flight_classes.fc_id', '=', 'flight_list.fl_fc_id')
                  ->join('airlines','airlines.airline_id', '=', 'flight_list.fl_airline_id')
                  ->get('fl_id');
